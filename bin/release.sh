...
# Store in an environment variable named GITHUB_ACCESS_TOKEN
# in your ~/.zshrc or ~/.bash_profile.
[ -z "$GITHUB_ACCESS_TOKEN" ] && { echo "Unable to read GITHUB_ACCESS_TOKEN environment variable."; exit 1; }

# The slug of your WordPress.org plugin
PLUGIN_SLUG="civil-comments"

# GITHUB user who owns the repo
GITHUB_REPO_OWNER="reaktivstudios"

# GITHUB Repository name
GITHUB_REPO_NAME="civil-comments"
...
# COPY ASSETS TO SVN
echo "Copying wporg-assets to SVN"
cd $ROOT_PATH$TEMP_SVN_REPO
cp -R "$ROOT_PATH$TEMP_GITHUB_REPO/wporg-assets/" assets/
rm -Rf "$ROOT_PATH$TEMP_GITHUB_REPO/wporg-assets/"
